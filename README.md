...
- [Feuille de route (ou juste des idées)](#feuille-de-route-ou-juste-des-idees)
...
## Feuille de route (ou juste des idées)

- [ ] Reprendre le CSS et tester l'ensemble du markup markdown
- [ ] Ajouter des tests
- [ ] Migrer sur sqlite-vec (plutôt que sqlite-vss) ?
...
- [ ] Refactoriser le code (là où nécessaire)
- [ ] Disable submit tant que le LLM n'a pas retourné sa réponse
- [ ] Sauvegarder la consommation de `tokens` dans la BDD (et le modèle ?)
- [ ] Tronquer les réponses si le nombre de `tokens` dépasse le `context` ?
- [ ] Permettre à l'utilisateur de noter les réponses (+/-) (+ KPI)
- [ ] Dark mode
...
Copyright (C) 2024, Charles-Henri Arnould (charnould@beckrel.com) et les contributeurs.

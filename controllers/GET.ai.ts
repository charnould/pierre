...
export const controller = async (c: Context) => {
  try {
    //
    // Build AI context
    let ai_context = AIContext.parse({
      role: 'user',
      uuid: c.req.param('id') as string,
      config: c.req.query('config') || 'default',
    ...
    ai_context = await AIContext.parseAsync({ ...ai_context, ...get_conversation(ai_context.uuid) })
    ai_context = AIContext.parse({ ...ai_context, ...(await enhance_query(ai_context)) })

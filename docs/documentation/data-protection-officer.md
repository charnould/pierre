...
Pour permettre à PIERRE de générer la réponse ou le courrier le plus pertinent, l'utilisateur peut joindre/ajouter au message du client des éléments de contexte complémentaires, soit (1) sous la forme de texte, soit (2) sous la forme de fichiers PDF : [démonstration](https://www.youtube.com/watch?v=U3r_BBxpsro) (vidéo YouTube).
...
#### Etape 1. Récupération des données pertinentes dans l'applicatif cible

- Récupération des données pertinentes dans le code-source web de l’application cible (= à titre d'exemple le message du client).

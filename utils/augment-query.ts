...
import { z } from 'zod'
import { Augmented_Query, type AIContext } from './_schema'
...
export const augment_query = async (context: AIContext) => {
  ...
  context.conversation.push({
    role: 'system',
    content: `
You are the ultimate AI assistant, specifically designed to handle social housing and related queries with unprecedented nuance and precision.
Your core mission is to analyze, structure, and enrich user input, leveraging all available data points across the conversation.
Your responses must be contextually accurate, deeply personalized, and capable of handling complex multi-part inquiries.
Every interaction should enhance both immediate and future assistance, laying a foundation for an ongoing conversational experience.

# Task 1: Language Detection

Determine the last language of this sentence: ${context.conversation[context.conversation.length - 1].content}

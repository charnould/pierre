...
export const rank_chunks = async (
  ...
  // TODO: adding them might not be a good idea.
  // Instead, validate relevancy with bm25?
  const vect_chunks = flatten_vector_searches(vect_data)
  const bm25_chunks = flatten_vector_searches(bm25_data) // _.differenceBy(flatten_vector_searches(bm25_data), vect_chunks, 'rowid')
  // const chunks = [...vect_chunks, ...bm25_chunks]
  const chunks = vect_chunks
  ...
  const scored_chunks = chunks.map((chunk, index) => ({
  ...
  if (Bun.env.DEBUG) {
    const print = async (data) => await prettier.format(JSON.stringify(data), { parser: 'json' })
...
export const pick_relevant_chunks = (scores): Relevant_Chunks =>
  _.chain(scores)
    .map((chunk) => ({
      ...chunk,
      global_score: _.sum([
        chunk.score.building_score,
        chunk.score.process_score,
        chunk.score.relevancy_score
      ])
    }))
    .filter((o) => o.global_score > 300)
    .groupBy('source')
    .mapValues((array) => _.chain(array).orderBy('global_score', 'desc').map('chunk_text').value())
...
You are an advanced semantic relevance evaluator with expertise in nuanced text analysis and contextual comprehension. Your task is to evaluate how effectively a given chunk of text answers a user query, assigning relevance scores based on a deep understanding of semantic alignment, precision, and context.

...
export const query_db = async (db_name: Db_Name, query: string, entities?) => {
  ...
  if (results.length === 0) {
    console.debug('No relevant `entities` identified, full db search.')
    ...
    console.debug('Relevant `entities` identified, narrow db search.')
    ...
    if (user_query.length === 0) console.debug('No result with narrow db search, full db search.')
    user_query = search_without_prefiltering(query_vectors, db_instance)
  }

...
document.addEventListener('DOMContentLoaded', () => {
  ...
  let q = document.getElementById('pierre-ia')
  ;((I = q?.dataset.configuration ?? 'default'),
    (J = q?.dataset.url ?? 'https://assistant.pierre-ia.org'))
...
document.addEventListener('click', (j) => {
  if (!document.getElementById('pierre-ia')) return
  if (j.target?.id === 'pierre-ia') {
    let z = document.createElement('div')
    ;((z.id = 'pierre-wrapper'), (z.className = 'blur_in'))
    let E = document.createElement('div')
    E.id = 'pierre-iframe-container'
    let B = document.getElementById('pierre-iframe')
    if (!B) return
    ;((B.style.display = 'unset'), (B.className = 'open_modal'))
  ...
  if (j.target?.id === 'pierre-wrapper' || j.target?.id === 'pierre-close') K()
})
...
var K = () => {
  ...
  let j = document.getElementById('pierre-wrapper'),
    q = document.getElementById('pierre-iframe')
  if (!j || !q) return
  ;((j.className = 'blur_out'),
    (q.className = 'close_modal'),
    setTimeout(() => {

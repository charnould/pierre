...
describe('UnreadPingIndicator', () => {
  test('uses the unread pastille token', async () => {
    const { act } = await import('react')
    const { createRoot } = await import('react-dom/client')
    const { UnreadPingIndicator } = await import('./UnreadPingIndicator')

    const host = document.createElement('div')
    document.body.append(host)
    const root = createRoot(host)
    await act(async () => {
      root.render(<UnreadPingIndicator />)
    })

    const ping = host.querySelector('[aria-hidden]')
    expect(ping?.className).toContain('bg-unread')
    expect(ping?.className).not.toContain('bg-foreground')

...
describe('chat drop files', () => {
  ...
  test('accepts supported files and rejects unsupported files', () => {
    const result = mergeChatDropFiles([], [file('photo.png'), file('archive.zip')])
  ...
  test('deduplicates by filename and size', () => {
    ...
    const result = mergeChatDropFiles([original], [file('rapport.pdf', 4)])
  ...
  test('enforces per-file, count and total limits', () => {
    const tooLarge = mergeChatDropFiles([], [file('grand.pdf', 10 * 1024 * 1024 + 1)])
  ...
  test('includes files already uploaded in the conversation quotas', () => {
    const count = mergeChatDropFiles([], [file('new.pdf')], { files: 5, bytes: 5 })
    expect(count.files).toEqual([])
    expect(count.errors).toEqual(['Vous pouvez joindre au maximum 5 fichiers.'])

    const bytes = mergeChatDropFiles([], [file('new.pdf')], {
      files: 1,
      bytes: 20 * 1024 * 1024
    })

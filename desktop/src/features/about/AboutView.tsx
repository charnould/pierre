...
import { ArtifactToolbarActions } from '@/features/workflow/components/WorkflowPanelChrome'
...
export function AboutView({ hidden, settings, onNavigate, agentName }: Props) {
  ...
  const {
    ...
    url: settings.url,
    id_skill,
    onNavigate: (tab) => onNavigateRef.current(tab),
    resetForm
  })
  ...
  const handleOutputChange = useCallback((v: string) => patchState({ output: v }), [patchState])
  ...
  const outputActions = (
    <ArtifactToolbarActions
      ...
      onCopy={(text, setCopied) => void copyText(text, setCopied)}
      exportDisabled
      dockEmbedded
          ...
          >
            ...
            <DeskPane id={WORKFLOW_PANEL_OUTPUT} minSize="40%">
              <AboutOutputPanel
                ...
                outputEditorKey={`about-output-${generationKey}`}

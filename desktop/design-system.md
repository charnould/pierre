...
**Impayés** — la dette est la **première ligne méta** (label « Dette », montant · équivalent loyer `tabular-nums`, flèche de tendance dans la valeur). Ordre : Dette · Courriel · Téléphone · Référent · Groupe · Dernière action · Tags. Seuls les libellés encore dans `customization/repayments/config.ts` s’affichent. Chips tags = une paire hex unique (`#E8E8E8` / `#333333`), pas de palette auto, pas de Coloriser DualTable. Pas de date de bail (colonne ledger).
...
**Réclamations** — même pile, même shell, mêmes labels 12 / contrôles 14. Ordre : `Répondre au locataire` · `Générer un point de situation` · `Ajouter une note` · `Créer une tâche` · `Consigner une action réalisée` · `Changer de panier` · `Importer un email` · `Changer les tags` · `Affecter à un référent`. `Répondre au locataire` ouvre un composeur unique : Format (`Via {application configurée}` si sa configuration est complète · `SMS / RCS` · `Courriel` · `Via la poste`) → Objet sauf SMS / RCS et format externe → Message. Le format externe, toujours courriel-like, est sélectionné par défaut lorsqu’il est disponible, sinon le courriel. Le champ Message occupe toute la hauteur restante du présent, avec un minimum éditable ; pendant la génération, il utilise exactement `AgentWorkTrace` + `GeneratedMarkdown` comme le module Synthèse, puis redevient une textarea éditable. Footer : `Rédiger avec {agent configuré}` · `Exporter en DOCX` pour le postal · `Annuler` · une seule primaire filled : `Injecter dans {application configurée}` pour le format externe, `Envoyer` pour les formats directs. L’application externe est configurée par module (`name`, `url_pattern`, `message_selector`) et s’ouvre hors React dans une fenêtre Electron modale ; après fermeture, une confirmation journalise ou non la réponse au locataire, même sans coordonnée connue, sans perdre le brouillon.
...
Communications (Impayés et Réclamations) = pied de card, pas L1.

| Ontologie         | Corps                                                                                                                                                                                                                                         |
| ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Contenu           | Surface filetée (citation), pas un Card                                                                                                                                                                                                       |
| Courriel          | Surface filetée : objet visible ; corps repliable, fermé par défaut (réception initiale d’une réclamation : ouvert) ; état d’acheminement en pied (11, muted). Import `.eml` : même card, pied `De … · À … · Envoyé le …`, pas d’acheminement |
...
Une entrée courriel (Impayés et Réclamations) est une phrase L2 : acteur + verbe au passé (Impayés : « a » + chip du libellé `action` + « par e-mail » — le médium vient du `type` d’activité, pas du `channel` du modèle : après confirmation, un modèle `mailto` crée une communication externe de type `email`). L1 = date seule. Le corps est une surface filetée : objet toujours visible ; corps du message repliable, fermé par défaut — **sauf** la réception initiale d’une réclamation, ouverte. En pied, une ligne 11 muted `Envoyé vers … · état au …` (RCS, courrier et autres communications : même pied). Pas de checkbox ni de Badge de statut. Un courriel importé (`.eml`) : L2 « a importé un courriel » ; L1 = date d’upload ; même card objet/corps ; pied `De … · À … · Envoyé le JJ/MM/AAAA · HHhMM` en clair. Pas de Dialog. Pas de drag-and-drop.

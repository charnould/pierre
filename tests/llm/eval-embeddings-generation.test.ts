...
import {
  generate_embeddings_with_hf,
  generate_embeddings_with_ollama
} from '../../utils/search-by-vectors'
...
it('should generate embeddings with Ollama', async () => {
  ...
  for (let t = 0; t < tests; t++) {
    ...
    await generate_embeddings_with_ollama(strings, true)
...
it.skip('should generate embeddings with Hugging Face', async () => {
  console.log('----------------')
  ...
  for (let t = 0; t < tests; t++) {
    ...
    for await (const string of strings) {
      const r = await generate_embeddings_with_hf(string)
      console.log(r)
    }
    const t1 = performance.now()
    const duration = t1 - to

...
test('evaluate query augmentation perf.', async () => {
  ...
  const context = {
    ...
    conversation: [
      ...
      { role: 'user', content: "qui est d'astreinte aujourd'hui " }
      ...
      // { role: 'user', content: 'qui es tu ? C koi un logement social, et combien y en a en france ?' },
      // { role: 'assistant', content: 'Rendez-vous sur le site dédié' },
      // { role: 'user', content: "je cherche un logement, c'est pour un T3 près de Paris" },
      // { role: 'assistant', content: "entendu, je réfléchis" },
      // { role: 'user', content: "c'est important, je suis célibataire avec 2 enfants à charge" }
    ]
  }
